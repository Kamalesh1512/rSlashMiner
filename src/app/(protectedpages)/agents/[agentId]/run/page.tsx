...
import { Loader2, CheckCircle, AlertCircle, ArrowLeft, XCircle, Clock } from "lucide-react"
      ...
      try {
        ...
        if (data.type === "step") {
          ...
          setSteps((prevSteps) => {
            ...
            const existingStepIndex = prevSteps.findIndex((step) => step.id === data.id)
            ...
            if (existingStepIndex >= 0) {
              ...
              updatedSteps[existingStepIndex] = {
                ...
                details: data.details || updatedSteps[existingStepIndex].details,
                timestamp: new Date(),
              }
              return updatedSteps
            } else {
        ...
        <CardContent className="space-y-6">
          {!isRunning && steps.length === 0 && !runResult ? (
            ...
            <div className="space-y-6">
                      ...
                      <AccordionTrigger className="px-4 py-3 hover:no-underline">
                        <div className="flex items-center gap-3 w-full">
                          <div className="flex-shrink-0">
                            ...
                            {step.status === "waiting" && <Clock className="h-5 w-5 text-muted-foreground" />}
                          </div>
                          <div className="flex-grow">
                            <div className="font-medium">{step.message}</div>
                          </div>
                          <div className="text-xs text-muted-foreground">{formatTime(step.timestamp)}</div>
                        </div>
                      </AccordionTrigger>
              ...
              {!isRunning && runResult && (
                <div
                  className={`p-4 rounded-lg border ${runResult.success ? "border-green-200 bg-green-50 dark:bg-green-900/10 dark:border-green-900/30" : "border-red-200 bg-red-50 dark:bg-red-900/10 dark:border-red-900/30"}`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    {runResult.success ? (
                      <>
                        <CheckCircle className="h-5 w-5 text-green-500" />
                        <h3 className="font-medium">Run Completed Successfully</h3>
                      </>
                    ) : (
                      <>
                  ...
                  <p className="text-sm">{runResult.success ? runResult.summary : runResult.error}</p>
                </div>
              )}
            </div>
          )}

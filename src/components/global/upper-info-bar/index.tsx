...
const UpperInfoBar = ({ user }: UpperInfoBarProps) => {
  ...
  const getPageTitle = () => {
    ...
    if (pathname.startsWith("/agents/") && pathname.includes("/results")) return "Agent Results"
    if (pathname.startsWith("/agents/") && pathname.includes("/settings")) return "Agent Settings"
    if (pathname.startsWith("/agents/") && pathname.includes("/create")) return "Create Agent"
            ...
            <div className="max-h-[300px] overflow-auto">
              <DropdownMenuItem className="flex flex-col items-start cursor-pointer">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Your agent "SaaS Monitor" found a new potential customer in r/startups
                </p>
              </DropdownMenuItem>
              <DropdownMenuItem className="flex flex-col items-start cursor-pointer">
                <div className="flex items-center justify-between w-full">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  "Product Hunt Monitor" completed its scheduled run with 2 new matches
                </p>
              </DropdownMenuItem>
              <DropdownMenuItem className="flex flex-col items-start cursor-pointer">
                <div className="flex items-center justify-between w-full">
